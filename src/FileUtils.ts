import {get_dir, create_dir} from './FolderUtils'
import {get_directory_name_from_path, get_filename_from_path} from './PathUtils'
 ...
 */
export function get_file(path: string, document_cls: any) {
  const dir = get_dir(get_directory_name_from_path(path));
 ...
 */
export function get_or_create_file(path: string, document_cls: any): object {
  const dir_name = get_directory_name_from_path(path);
  const file_name = get_filename_from_path(path);
  const dir = get_dir(dir_name) || create_dir(dir_name);
 ...
 */
export function create_file(path: string, document_cls: any): object {
  const dir_name = get_directory_name_from_path(path);
  const file_name = get_filename_from_path(path);
  const dir = get_dir(dir_name) || create_dir(dir_name);
  const files = dir.getFilesByName(file_name);
